        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                if isSearching {
                    ...
                    if showingMap {
                        MapResultsView(cities: searchResults, selectedCity: $selectedCity, showingDetail: $showingDetail)
                    } else {
                        resultsList
                    }
                }
        ...
        .toolbar {
            ToolbarItem(placement: .primaryAction) {
                Button(action: {
                    ...
                    Label(showingMap ? "List View" : "Map View",
                          systemImage: showingMap ? "list.bullet" : "map")
                }
            }
        }
        .sheet(isPresented: $showingDetail) {
        ...
        NavigationView {
            List {
                ...
                Section("Coordinates") {
                    ...
                    Button(action: {
                        let newLat = String(format: "%.4f", city.lat)
                        let newLng = String(format: "%.4f", city.lng)
                        print("📍 Updating coordinates to: \(newLat), \(newLng) from city: \(city.name)")
                        spatialLat = newLat
                        spatialLng = newLng
                        coordinatesUpdated = true
                        print("📍 Updated! spatialLat=\(spatialLat), spatialLng=\(spatialLng)")
                    }) {
...
// Extension to make CityResult Identifiable for use in Map
extension CityResult: Identifiable {
    public var id: String {
        "\(name)-\(lat)-\(lng)"
    }
}

// Map view for showing search results on a map
struct MapResultsView: View {
    let cities: [CityResult]
    @Binding var selectedCity: CityResult?
    ...
    var body: some View {
        Map(coordinateRegion: $region, annotationItems: cities, annotationContent: { city in
    ...
    private func updateRegion() {
        ...
        let span = MKCoordinateSpan(
            latitudeDelta: max(maxLat - minLat, 0.1) * 1.5,
            longitudeDelta: max(maxLng - minLng, 0.1) * 1.5
        )
