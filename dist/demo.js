...
function renderCard(hit) {
    ...
    // We use encodeURIComponent to ensure quotes don't break the HTML attribute
    const jsonSafe = encodeURIComponent(JSON.stringify(hit));

    return `
    <div class="result-card" onclick="selectHit('${jsonSafe}')" title="Click to use coordinates">
...
// ---------------------------------------------------------
// 2. Interaction Handler (Updated)
// ---------------------------------------------------------
window.selectHit = function(encodedJson) {
    try {
        const hit = JSON.parse(decodeURIComponent(encodedJson));
        ...
        // 1. Show Details with a Close Button
        ...
        detailBox.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #555; padding-bottom:10px; margin-bottom:10px;">
                <h3 style="margin:0; color:#fff;">📍 Selected: ${hit.name}</h3>
                <button onclick="clearDetail()" style="background:#dc3545; padding:5px 10px; font-size:12px; margin:0;">Close</button>
            </div>
            <pre>${JSON.stringify(hit, null, 2)}</pre>
        `;

        // 2. Auto-fill Coordinates (Robust Check)
        let lat = hit.latitude != null ? hit.latitude : hit.lat;
        let lng = hit.longitude != null ? hit.longitude : hit.lng;
        ...
        if (lat != null && lng != null) {
            ...
            // Visual Cue
            latInput.parentElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
            latInput.style.backgroundColor = '#e3f2fd';
            lngInput.style.backgroundColor = '#e3f2fd';
            // Keep the highlight until cleared or changed
        } else {
            latInput.value = '';
            lngInput.value = '';
        }
        ...
        console.error("Error selecting hit:", e);
    }
};
...
window.clearDetail = function() {
    ...
    // Clear spatial results too? Optional, but cleaner.
    document.getElementById('spatialOut').innerHTML = '';
};
...
window.loadEngine = async function(jsPath, label) {
    ...
    try {
        ...
        if (module.run_app) {
            ...
            // Fallback for older/other modules
            applyHashParams();
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadEngine('/js/flat-blobs/geodb_wasm.js', 'Flat (Flat + Blobs)');
...
// ---------------------------------------------------------
// 4. Hash Fragment Handling (#q=...&pop=...)
// ---------------------------------------------------------
function applyHashParams() {
    ...
    const pop = parseInt(params.get('pop')) || 0;

    if (q) {
        const input = document.getElementById('smartInput');
        const out = document.getElementById('smartOut');
        input.value = q;
        
        const results = window.currentModule.smart_search(q);
        // Apply population filter if present
        const filtered = pop > 0 ? results.filter(r => r.kind === 'city' && (r.population || 0) > pop) : results;
        
        const html = filtered.slice(0, 10).map(renderCard).join('');
        out.innerHTML = `<div style="color:#28a745; margin-bottom:10px"><b># Fragment Auto-Search:</b> ${filtered.length} hits (Min Pop: ${pop})</div>` + html;
        
        // If a city is found, automatically run spatial search for nearby cities > 20k
        const targetCity = filtered.find(r => r.kind === 'city');
        if (targetCity) {
            console.log("Auto-Spatial Search triggered for:", targetCity.name);
            const nearest = window.currentModule.find_nearest_cities(targetCity.latitude, targetCity.longitude, 20);
            const filterVal = Math.max(pop, 20000);
            const top10 = nearest.filter(c => (c.population || 0) > filterVal).slice(0, 10);
            
            const spatialOut = document.getElementById('spatialOut');
            spatialOut.innerHTML = `<h3>Top 10 Cities near ${targetCity.name} with Population > ${filterVal}</h3>` + 
                                  top10.map(renderCard).join('');
        }
    }
}
