...
# Output: GeoDB-Apps/SPM GeoDBKit/
#
...
cargo run --bin uniffi-bindgen generate \
    --library "$TARGET_DIR/aarch64-apple-darwin/$CARGO_PROFILE/libgeodb_ffi.dylib" \
    --language swift \
    --out-dir generated
...
# Use the comprehensive SPM build script from GeoDB-Apps/scripts
SPM_BUILD_SCRIPT="$ROOT_DIR/GeoDB-Apps/scripts/build_spm_package.sh"
...
if [ -f "$SPM_BUILD_SCRIPT" ]; then
    ...
    echo "  2. Copy it to: $SPM_PACKAGE/GeodbFfi.xcframework"
...
echo "  2. Add 'SPM GeoDBKit' as local package"
