...
FLUTTER_PUBSPEC="$ROOT_DIR/GeoDB-Apps/geodb_flutter/pubspec.yaml"
...
FLUTTER_EXAMPLE_PUBSPEC="$ROOT_DIR/GeoDB-Apps/geodb_flutter/example/pubspec.yaml"
if [ -f "$FLUTTER_EXAMPLE_PUBSPEC" ]; then
    ...
    if grep -q "geodb_flutter:" "$FLUTTER_EXAMPLE_PUBSPEC"; then
...
echo ""
echo -e "${GREEN}►${NC} Python (geodb-py)"
echo "  ℹ  Version automatically synced from crates/geodb-py/Cargo.toml"
...
echo "  • GeoDB-Apps/geodb_flutter/pubspec.yaml"
echo ""
echo "Next steps:"
echo ""
