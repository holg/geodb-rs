...
if [ "$OUTPUT_TYPE" = "AAB" ]; then
    ...
    if [ -f "$ANDROID_APP/app/build/outputs/mapping/release/mapping.txt" ]; then
        echo "Copying ProGuard mapping file..."
        cp "$ANDROID_APP/app/build/outputs/mapping/release/mapping.txt" "$RELEASES_DIR/"
