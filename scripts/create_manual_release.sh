...
SPM_DIR="$ROOT_DIR/GeoDB-Apps/SPM-GeoDBKit"
...
if [ ! -d "$SPM_DIR/GeodbFfi.xcframework" ]; then
    echo -e "${YELLOW}Error: GeodbFfi.xcframework not found${NC}"
...
# Package name
RELEASE_ZIP="$RELEASES_DIR/GeodbKit-$VERSION-universal.zip"
...
TEMP_STAGE="/tmp/geodbkit-release-$$"
rm -rf "$TEMP_STAGE"
mkdir -p "$TEMP_STAGE/GeodbKit"

# Copy essential files (RELEASE ONLY - no debug)
echo "Copying files..."
cp -R "$SPM_DIR/GeodbFfi.xcframework" "$TEMP_STAGE/GeodbKit/"
cp -R "$SPM_DIR/Sources" "$TEMP_STAGE/GeodbKit/"
cp "$SPM_DIR/Package.swift" "$TEMP_STAGE/GeodbKit/"
cp "$SPM_DIR/README.md" "$TEMP_STAGE/GeodbKit/"

echo "Note: Debug XCFramework NOT included (saves 1.1GB)"

echo "Creating ZIP..."
cd "$TEMP_STAGE"
zip -r "$RELEASE_ZIP" GeodbKit -q
